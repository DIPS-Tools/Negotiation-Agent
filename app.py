...
#recieve new preference from data consumer or provider
def new_preference(preference : Preference) -> Proposal | None:
    result = None
    return result

#recive new proposal from other agent
def new_proposal(proposal : Proposal) -> Proposal | None:
    return None
...
def new_request(record : RecommenderAgentRecord) -> Proposal:
    ...
    if record.previous_request is None:
        result = Proposal(
                ...
                party = record.preference.upper_third_party)
    else:   
